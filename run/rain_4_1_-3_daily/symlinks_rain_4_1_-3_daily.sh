...
input_basepath=/pool/data/ECHAM6-HAMMOZ/${input_files_version}      # where all input files except nudging data is to be found
nudg_basepath=/pool/data/nudging/eraia                              # where all nudging data is to be found
...
#------------------------------
  ...
  #-- sst_sic_dataset
      ...
      # uncomment the following line to override the automatic setting:
      sst_sic_dataset=amip2_plus_4K

      possible_values=( amip2 amip2_plus_4K ) # add additional values in case you want to use an alternate sst/sic dataset
...
if $flag_time_dep_sst_sic ; then
   ...
   elif [[ "$sst_sic_dataset" == "amip2_plus_4K" ]] ; then # use XXXX data

       for ((year=$start_year_m1; year<=$stop_year_p1; year++)) ; do
           ln -sf ${HOME}/echam6_input/${hres}/amip2_plus_4K/${hres}_amip2sst_${year}.nc sst${year}
           ln -sf ${HOME}/echam6_input/${hres}/amip2_plus_4K/${hres}_amip2sic_${year}.nc ice${year}
       done
   fi
...
if $flag_nudg ; then
   ...
   while ((idate<=$stop_yyyymm)) ; do
       ...
       for prefix in ${nudg_prefix_order[*]} ; do
           if [ -d ${nudg_basepath}/${prefix}/${prefix}${hres}${vres}${dir_ext} ] ; then
       ...
       if $flag_not_found ; then
          ...
          nudg_subdir=`find ${nudg_basepath} -maxdepth 2 -type d \
                       -name "*${hres}${vres}${dir_ext}" 2> /dev/null | sort | head -1`
          ...
          prefix=${prefix%%${hres}${vres}${dir_ext}}
       fi
       ...
       nudg_name="${prefix}${hres}${vres}"

       #-- make the links:
       for ext in ${exts[*]} ; do
           ln -sf ${nudg_basepath}/${nudg_name}${dir_ext}/*_${idate}.${ext} ndg${idate}.${ext}
       done
       ...
       if [[ "$idate" -eq "$exp_start_yyyymm" ]] ; then
          ...
          for ext in ${exts[*]} ; do
              ln -sf ${nudg_basepath}/${nudg_name}${dir_ext}/*_${idate}.${ext} ndg${exp_start_yyyymm_m1}.${ext}
          done

       elif [[ "$idate" -eq "$stop_yyyymm" ]] ; then

          for ext in ${exts[*]} ; do
              ln -sf ${nudg_basepath}/${nudg_name}${dir_ext}/*_${idate}.${ext} ndg${stop_yyyymm_p1}.${ext}
          done
